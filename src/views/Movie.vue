<template>
  ...
  <main class="">
    <div
      class="cards row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xl-5"
    >
...
const handleSearch = async () => {
  movieList.value = await fetch(
    `https://api.themoviedb.org/3/search/movie?query=${search_query.value}&page=1&include_adult=false&api_key=45fcb150351f5049358ecdb91b91b4e0`
  )
    .then((res) => res.json())
    .then((data) => data.results); //  위 then의 리턴값을 data로 받아옴
